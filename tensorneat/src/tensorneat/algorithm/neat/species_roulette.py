...
class SpeciesControllerRoulette(StatefulBaseClass):
    def __init__(
        self,
        pop_size,
        species_size,
    ...
    def _create_crossover_pair(self, species_state, randkey, spawn_number, fitness):
        ...
        def aux_func(key, idx):
            ...
            sorted_fitnesses = jnp.sort(members_fitness)[::-1]
            ...
            min_fitness = jnp.min(fitness_shifted, where=valid, initial=0.0)
            fitness_shifted = jnp.where(valid, sorted_fitnesses - min_fitness + 1e-8, 0.0)
            select_pro = fitness_shifted / jnp.sum(fitness_shifted)

            # # Get the fitnesses of the top survivors
            # survivor_fitness = jax.lax.dynamic_slice_in_dim(sorted_fitnesses, 0, survive_size)

            # # Shift fitness to be non-negative (if needed)
            # min_fitness = jnp.min(survivor_fitness)
            # fitness_shifted = survivor_fitness - min_fitness + 1e-8  # avoid zero division

            # # Normalize to get probabilities
            # probs = fitness_shifted / jnp.sum(fitness_shifted)

            # # Build a probability vector for the whole population (zeros for non-survivors)
            # select_pro = jnp.zeros(self.pop_size)
            # select_pro = select_pro.at[:survive_size].set(probs)
